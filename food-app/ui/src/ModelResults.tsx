...
export function ModelResults({ onOpen }: { onOpen: (path: string) => void }) {
  ...
  return <section className="library model-results" aria-label="Model results">
    ...
    <p className="caption">Processing success does not verify recipe fidelity. Compare book coverage and review flags before drawing conclusions. Attempts and costs belong to the displayed run; historical unknowns stay unknown.</p>
    ...
    {rows.length > 0 && <div className="model-results-scroll" tabIndex={0} role="region" aria-label="Model comparison table">
      <table>
        ...
        <thead><tr><th scope="col">Book / model</th><th scope="col">Success</th><th scope="col">Book coverage</th><th scope="col">Recipes / review</th><th scope="col">Attempts</th><th scope="col">New estimated spend</th><th scope="col">Extraction</th></tr></thead>
        <tbody>{rows.map((row) => {
          const r = row.latest;
          return <tr key={r.path}>
            ...
            <td>{r.recipes} recipes<span>{`${row.contentReviewFlags} content review ${row.contentReviewFlags === 1 ? "flag" : "flags"}`}</span></td>
