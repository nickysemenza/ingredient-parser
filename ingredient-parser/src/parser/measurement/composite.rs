...
impl<'a> MeasurementParser<'a> {
    ...
    /// Parse "`<count> (<size>) <container>`" such as "1 (1-ounce) piece" or
    /// "1 (28-ounce) can", producing `[<count> <container>, <inner size>]` —
    /// e.g. "1 (1-ounce) piece ginger" → `[1 piece, 1 oz]`.
    ///
    /// Only fires when the parenthetical is a *hyphenated* size adjective
    /// ("1-ounce", "13.5-gram") that fully parses as a measurement; the space
    /// form "(14.5 oz)" is left to [`parse_parenthesized_amounts`], and a
    /// container noun is required so arbitrary parentheticals like
    /// "(not defrosted)" don't match.
    pub(super) fn parse_count_with_parenthetical_size<'b>(
        ...
        // Only the hyphenated "1-ounce" form; "(14.5 oz)" stays with the
        // existing parenthesized-amount path.
        if !inner.contains('-') {
            return Err(reject());
        }

        // Require a container noun immediately after the parenthetical.
