...
/// Scaling an ingredient changes its count while preserving dimensional prose.
#[test]
fn pie_crust_dimension_is_not_scaled() {
    let parsed = from_str("1 (9-inch) pie crust");
    let doubled = parsed.scale(2.0);
    assert_eq!(doubled.modifier, parsed.modifier);
    assert_eq!(doubled.modifier.as_deref(), Some("9-inch"));
    assert_eq!(doubled.amounts.len(), 1);
    assert_eq!(*doubled.amounts[0].unit(), Unit::Whole);
