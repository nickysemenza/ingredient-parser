...
fn benchmark_ingredient_parsing(c: &mut Criterion) {
    ...
    for (name, input) in &test_cases {
        ...
        group.bench_with_input(BenchmarkId::new("from_str", name), input, |b, input| {
            ...
            b.iter(|| parser.clone().from_str(std::hint::black_box(input)))
        });
        ...
        group.bench_with_input(
            ...
            |b, input| {
                ...
                b.iter(|| rich_parser.clone().from_str(std::hint::black_box(input)))
            },
        );
        ...
        group.bench_with_input(BenchmarkId::new("try_from_str", name), input, |b, input| {
            b.iter(|| {
                let parser = IngredientParser::new(false);
                parser.try_from_str(std::hint::black_box(input))
            })
        });
    }
...
fn benchmark_amount_parsing(c: &mut Criterion) {
    ...
    for (name, input) in &amount_cases {
        group.bench_with_input(BenchmarkId::new("parse_amount", name), input, |b, input| {
            let parser = IngredientParser::new(false);
            b.iter(|| parser.clone().parse_amount(std::hint::black_box(input)))
        });
        ...
        group.bench_with_input(
            ...
            |b, input| {
                ...
                b.iter(|| {
                    parser
                        .clone()
                        .must_parse_amount(std::hint::black_box(input))
                })
            },
        );
    }
...
fn benchmark_parsing_vs_creation(c: &mut Criterion) {
    ...
    c.bench_function("create_parser", |b| {
        b.iter(|| IngredientParser::new(std::hint::black_box(false)))
    });

    c.bench_function("parse_with_existing", |b| {
        let parser = IngredientParser::new(false);
        b.iter(|| parser.clone().from_str(std::hint::black_box(test_input)))
    });

    c.bench_function("parse_with_creation", |b| {
        b.iter(|| {
            let parser = IngredientParser::new(false);
            parser.from_str(std::hint::black_box(test_input))
        })
    });
}
...
fn benchmark_batch_parsing(c: &mut Criterion) {
    ...
    c.bench_function("batch_parse_recipe", |b| {
        ...
        b.iter(|| {
            for ingredient in &recipe_ingredients {
                parser.clone().from_str(std::hint::black_box(ingredient));
            }
        })
    });
    ...
    c.bench_function("batch_parse_with_creation", |b| {
        b.iter(|| {
            for ingredient in &recipe_ingredients {
                let parser = IngredientParser::new(false);
                parser.from_str(std::hint::black_box(ingredient));
            }
        })
    });
}
