...
- **Where a fix goes:** the pipeline is normalize → recognize → grammar → refine. To see which stage shaped a line (and therefore where a fix belongs), run `parse-ingredient --explain "<line>"`. The routing decision tree lives in the `ingredient-parser/src/parser/mod.rs` module doc ("Where does a parser fix go?").
